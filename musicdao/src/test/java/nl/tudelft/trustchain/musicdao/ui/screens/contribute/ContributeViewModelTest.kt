...
class ContributeViewModelTest {
    ...
    fun setup() {
        ...
        every { IPv8Android.getInstance() } returns
            mockk {
    ...
    @Test
    fun `when no listen activity data exists, contribution should fail`() =
        runTest {
    ...
    @Test
    fun `when no payout node is found, contribution should fail`() =
        runTest {
    ...
    @Test
    fun `when contribution succeeds, should create proposal block and cache contribution`() =
        runTest {
    ...
    @Test
    fun `when payment transaction fails, contribution should fail`() =
        runTest {
            val listenData = mapOf("artist1" to 60.0, "artist2" to 30.0)
            every { listenActivityRepository.getMinutesPerArtist() } returns listenData
    ...
    @Test
    fun `when artists have bitcoin addresses in repository, they should be used in contribution split`() =
        runTest {
            ...
            every { listenActivityRepository.getMinutesPerArtist() } returns listenData
            every {
                payoutService.makeContribution(
                    100f,
                    match {
    ...
    @Test
    fun `when artist keys contain bitcoin address format, they should be used directly`() =
        runTest {
    ...
    @Test
    fun `contribute should correctly calculate proportions when multiple artists have different listen times`() =
        runTest {
            val listenData =
                mapOf(
                    "artist1" to 100.0,
    ...
    @Test
    fun `contribute should handle mixed artist identification formats`() =
        runTest {
            val listenData =
                mapOf(
                    "artist1" to 50.0,
    ...
    @Test
    fun `contribute should create proposal block with correct transaction data`() =
        runTest {
            val listenData =
                mapOf(
                    "artist1" to 100.0
    ...
    @Test
    fun `contribute should handle very small listen times correctly`() =
        runTest {
            val listenData =
                mapOf(
                    "artist1" to 0.001,
    ...
    @Test
    fun `contribute should clear listen activity data after successful contribution`() =
        runTest {
